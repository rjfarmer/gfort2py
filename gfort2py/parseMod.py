...
def parse_derived_type(info, dt_names, dt=False):
    ...
    # remove first bracket and some times this will have a space between the (
    # and derived
    x = x[1:].strip()
    if 'DERIVED' in x:
        sx = x.split()
...
def doStuff(filename):
    ...
    for i, j in zip(split_data[0::2], split_data[1::2]):
        ...
        if j[1] == "(":
            # These are the components of a derived type and 
            # allways come after the
            # initial defintion of the derived type
            sp = i.split()
            dt_defs[-1]['args'].append({'num': int(sp[0]),
                                        'name': sp[1], 'info': j})
        else:
            line = i + j
            sp = line.split()
            ...
            if d['parent'] > 1:
                ...
                # We dont call processDT as we still have to append the
                # contents at the end
                d['args'] = []
                dt_defs.append(d)
            elif 'PROCEDURE' in j:
