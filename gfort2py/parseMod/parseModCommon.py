...
class parseModBase(object):
    ...
    def parseAllDTDefines(self):
        ...
        for symbol in self.data[6]:
            ...
            if "DERIVED " in type_line:
                ...
                if not found:
                    self.dt_defines.append(
                        {"name": name, "module": module.replace("'", ""), "num": num}
                    )
    ...
    def parseAllNameSymbols(self):
        ...
        for i in range(len(x) // 3):
            self.symbol_names.append(
                {
                    "name": x[i * 3],
                    "num": x[i * 3 + 2],
                    "ambiguous_flag": bool(x[i * 3 + 1]),
                }
            )
    ...
    def getParamValue(self, x, typ):
        if "ARRAY" in x:
            ...
            if "COMPLEX" in x:
                yy = x.split("'")
                p = complex(
                    self.parseSingleParam(yy[-4], "float"),
    ...
    def matchFuncPtrs(self):
        for key in self.all_symbols:
            ...
            if "proc" in value and "var" in value:
                self.all_symbols[key]["arg"] = self.all_symbols[
                    value["var"]["proc_ptr_id"]
                ]["arg"]
                self.all_symbols[key]["proc"]["arg_nums"] = [-1] * len(
                    self.all_symbols[key]["arg"]
                )
